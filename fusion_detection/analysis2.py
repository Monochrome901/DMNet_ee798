...
def convert_txt_to_coco(annotation_dir, output_json_file):
    """
    Convert the .txt ground truth annotation files to a COCO-style JSON file.
    """
    images = []
    annotations = []
    ...
    for img_id, ann_file in enumerate(os.listdir(annotation_dir)):
        if ann_file.endswith('.txt'):
            ...
            img_info = {
                "file_name": img_name,
                "id": img_id  # Ensure each image has a unique ID matching predictions
            }
            ...
            with open(os.path.join(annotation_dir, ann_file), 'r') as f:
                for line in f:
                    ...
                    if len(elements) == 5:
                        ...
                        annotation = {
                            "image_id": img_id,  # Match the image ID with the predictions
                            "id": annotation_id,
...
convert_txt_to_coco('dataset/Global/val/annotations', 'ground_truth_annotations_coco.json')
