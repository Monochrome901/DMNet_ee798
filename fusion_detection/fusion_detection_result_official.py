...
if __name__ == "__main__":
    ...
    final_detection_by_image = {}
    for i, img_file in tqdm(enumerate(img_list), total=len(img_list)):
        current_nms_target = img_fusion_result_collecter[i]
        nms_preprocess = wrap_initial_result(current_nms_target)
        keep = class_wise_nms(nms_preprocess, args.iou_threshold, args.TopN)
        class_wise_nms_result = [current_nms_target[k] for k in keep]
        final_detection_result.extend(class_wise_nms_result)
        final_detection_by_image[img_name] = class_wise_nms_result
        overlay_func(img_file, class_wise_nms_result, classList, truncate_threshold, exclude_region=None, show=False)

    # Save fusion detection result to output json file
    output_file = os.path.join(root, "Global", mode, "final_fusion_result.json")
