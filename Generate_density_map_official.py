...
def generate_gaussian_kernels(out_kernels_path='gaussian_kernels.pkl', round_decimals=3, sigma_threshold=4, sigma_min=0,
    ...
    for sigma in tqdm(sigma_space):
        ...
        kernel_size = np.ceil(sigma * sigma_threshold).astype(np.int64)
...
def parse_args():
    ...
    parser.add_argument('root_dir', default=".", help='the path for source data')
    parser.add_argument('precomputed_kernels_path', nargs='?', default="gaussian_kernels.pkl", 
                        help='the path to save precomputed kernels (default: gaussian_kernels.pkl)')
    parser.add_argument('precomputed_distances_path', nargs='?', default="distances_dict.pkl", 
                        help='the path to save precomputed distance (default: distances_dict.pkl)')
...
if __name__ == "__main__":
    ...
    with open(str(mode) + ".txt", "w") as fileloader:
        ...
        for img_path in tqdm(img_paths):
            ...
            den_name = os.path.join(data_root, "dens", os.path.basename(img_path).replace("jpg", "npy"))
            if showden:
                plt.imshow(img)
